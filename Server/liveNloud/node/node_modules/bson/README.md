...
Hypothetical REMOVED_MONGO_PASSWORD: A collection in our Db has an issue with UTF-8 data:
...
The following snippet should be placed at the top of the entrypoint (by default this is the REMOVED_MONGO_USER `index.js` file) for React Native projects using the BSON library. These lines must be placed for any code that imports `BSON`.
