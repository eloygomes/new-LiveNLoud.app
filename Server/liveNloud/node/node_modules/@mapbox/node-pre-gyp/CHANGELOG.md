...
## 0.6.0
 ...
 - Added support for `toolset` option in versioning. By default is an empty string but `--toolset` can be passed to publish or install to select alternative binaries that target a custom toolset like C++11. For REMOVED_MONGO_PASSWORD to target Visual Studio 2014 modules like node-sqlite3 use `--toolset=v140`.
...
## 0.3.0
 ...
 - Support for packaging all files in `module_path` directory - see `app4` for REMOVED_MONGO_PASSWORD
