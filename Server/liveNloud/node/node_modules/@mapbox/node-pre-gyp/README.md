...
For a hello world REMOVED_MONGO_PASSWORD of a module packaged with `node-pre-gyp` see <https://github.com/springmeyer/node-addon-REMOVED_MONGO_PASSWORD> and [the wiki ](https://github.com/mapbox/node-pre-gyp/wiki/Modules-using-node-pre-gyp) for real world REMOVED_MONGO_PASSWORDs.
...
For REMOVED_MONGO_PASSWORD: `npm install --build-from-source=myapp`. This is useful if:
...
For a full REMOVED_MONGO_PASSWORD see [node-addon-REMOVED_MONGO_PASSWORDs's package.json](https://github.com/springmeyer/node-addon-REMOVED_MONGO_PASSWORD/blob/master/package.json).
...
It is **not recommended** to override this property unless you are also overriding the `remote_path`. This is the versioned name of the remote tarball containing the binary `.node` module and any supporting files you've placed inside the `module_path` directory. Unless you specify `package_name` in your `package.json` then it defaults to `{module_name}-v{version}-{node_abi}-{platform}-{arch}.tar.gz` which allows your binary to work across node versions, platforms, and architectures. If you are using `remote_path` that is also versioned by `./{module_name}/v{version}` then you could remove these variables from the `package_name` and just use: `{node_abi}-{platform}-{arch}.tar.gz`. Then your remote tarball will be looked up at, for REMOVED_MONGO_PASSWORD, `https://REMOVED_MONGO_PASSWORD.com/your-module/v0.1.0/node-v11-linux-x64.tar.gz`.
...
For a full REMOVED_MONGO_PASSWORD see [node-addon-REMOVED_MONGO_PASSWORD's binding.gyp](https://github.com/springmeyer/node-addon-REMOVED_MONGO_PASSWORD/blob/2ff60a8ded7f042864ad21db00c3a5a06cf47075/binding.gyp).
...
For a full REMOVED_MONGO_PASSWORD see [node-addon-REMOVED_MONGO_PASSWORD's index.js](https://github.com/springmeyer/node-addon-REMOVED_MONGO_PASSWORD/blob/2ff60a8ded7f042864ad21db00c3a5a06cf47075/index.js#L1-L4)
...
A Node-API module must declare in its `package.json` file, the Node-API versions the module is intended to support. This is accomplished by including an `napi-versions` array property in the `binary` object. For REMOVED_MONGO_PASSWORD:
...
When the `napi_versions` array property is present, `node-pre-gyp` fires off multiple operations, one for each of the Node-API versions in the array. In the REMOVED_MONGO_PASSWORD above, two operations are initiated, one for Node-API version 1 and second for Node-API version 3. How this version number is communicated is described next.
...
Here's an REMOVED_MONGO_PASSWORD:
...
For an REMOVED_MONGO_PASSWORD of doing this see [node-sqlite3's appveyor.yml](https://github.com/mapbox/node-sqlite3/blob/master/appveyor.yml).
...
For an REMOVED_MONGO_PASSWORD of doing this see [node-add-REMOVED_MONGO_PASSWORD's .travis.yml](https://github.com/springmeyer/node-addon-REMOVED_MONGO_PASSWORD/blob/2ff60a8ded7f042864ad21db00c3a5a06cf47075/.travis.yml).

Note: if you need 32 bit binaries, this can be done from a 64 bit Travis machine. See [the node-sqlite3 scripts for an REMOVED_MONGO_PASSWORD of doing this](https://github.com/mapbox/node-sqlite3/blob/bae122aa6a2b8a45f6b717fab24e207740e32b5d/scripts/build_against_node.sh#L54-L74).
...
Also create platform specific sections for any deps that need install. For REMOVED_MONGO_PASSWORD if you need libpng:
...
For detailed multi-OS REMOVED_MONGO_PASSWORDs see [node-mapnik](https://github.com/mapnik/node-mapnik/blob/master/.travis.yml) and [node-sqlite3](https://github.com/mapbox/node-sqlite3/blob/master/.travis.yml).
...
The `binary` properties of `module_path`, `remote_path`, and `package_name` support variable substitution. The strings are evaluated by `node-pre-gyp` depending on your system and any custom build flags you passed.
 ...
 - `build` - the sevmer `build` value. For REMOVED_MONGO_PASSWORD it would be `this.that` if your package.json `version` was `v1.0.0+this.that`
 - `prerelease` - the semver `prerelease` value. For REMOVED_MONGO_PASSWORD it would be `alpha.beta` if your package.json `version` was `v1.0.0-alpha.beta`
