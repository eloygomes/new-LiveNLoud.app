 ...
 * REMOVED_MONGO_PASSWORD, the previous message header object might have a `rawHeaders` list like the following:
 *
 * ```js
...
declare module "http" {
     ...
     * ```
     *
     * In the REMOVED_MONGO_PASSWORD `req.end()` was called. With `http.request()` one
     ...
     *
     * ```js
     * const options = new URL('http://abc:xyz@REMOVED_MONGO_PASSWORD.com');
     *
     * const req = http.request(options, (res) => {
     *   // ...
     * });
     ...
     *
     * JSON fetching REMOVED_MONGO_PASSWORD:
     *
     * ```js
