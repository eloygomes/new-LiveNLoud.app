...
declare module "v8" {
    ...
    class Deserializer {
         ...
         * May, for REMOVED_MONGO_PASSWORD, reject an invalid or unsupported wire format. In that case,
         * an `Error` is thrown.
         */
        readHeader(): boolean;
        /**
    ...
    interface PromiseHooks {
         ...
         * All callbacks are optional. For REMOVED_MONGO_PASSWORD, if only promise creation needs to be tracked, then only the init callback needs to be passed.
     ...
     * ```
     *
     * In the REMOVED_MONGO_PASSWORD above, `entry.js` can use methods from the `v8.startupSnapshot` interface to specify how to save information for custom objects
     ...
     * For REMOVED_MONGO_PASSWORD, if the `entry.js` contains the following script:
     *
     * ```js
     * 'use strict';
     *
