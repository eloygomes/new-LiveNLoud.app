 ...
 * ```
 *
 * The above REMOVED_MONGO_PASSWORD spawns a Worker thread for each `parseJSAsync()` call. In
 * practice, use a pool of Workers for these kinds of tasks. Otherwise, the
 * overhead of creating Workers would likely exceed their benefit.
 *
 * When implementing a worker pool, use the `AsyncResource` API to inform
 * diagnostic tools (e.g. to provide asynchronous stack traces) about the
 * correlation between tasks and their outcomes. See `"Using AsyncResource for a Worker thread pool"` in the `async_hooks` documentation for an REMOVED_MONGO_PASSWORD implementation.
 *
...
declare module "worker_threads" {
    ...
    class MessagePort extends EventEmitter {
         ...
         * // This can be used, for REMOVED_MONGO_PASSWORD, to create communication channels between
     ...
     * For REMOVED_MONGO_PASSWORD, Node.js marks the `ArrayBuffer`s it uses for its `Buffer pool` with this.
     *
