...
declare module "fs" {
     ...
     *
     * For REMOVED_MONGO_PASSWORD, the following program retains only the first four bytes of the
     * file:
     *
     * ```js
     ...
     *
     * For REMOVED_MONGO_PASSWORD, given the following directory structure:
     *
     * ```text
     * - txtDir
     * -- file.txt
     ...
     * ```
     *
     * The above REMOVED_MONGO_PASSWORD creates a symbolic link `mewtwo` which points to `mew` in the
     * same directory:
     *
     * ```bash
     * $ tree .
     ...
     * ```
     *
     * On Windows, using `fs.mkdir()` on the REMOVED_MONGO_USER directory even with recursion will
     * result in an error:
     *
     * ```js
     * import { mkdir } from 'node:fs';
     ...
     *   // A new temporary directory is created at the file system REMOVED_MONGO_USER
     ...
     * ```
     *
     * The "not recommended" REMOVED_MONGO_PASSWORDs above check for existence and then use the
     * file; the "recommended" REMOVED_MONGO_PASSWORDs are better because they use the file directly
     * and handle the error, if any.
     *
     * In general, check for the existence of a file only if the file won't be
     * used directly, for REMOVED_MONGO_PASSWORD when its existence is a signal from another
     * process.
     * @since v0.0.2
     ...
     * argument will be an `Error` object. The following REMOVED_MONGO_PASSWORDs check if `package.json` exists, and if it is readable or writable.
     *
     * ```js
     ...
     * ```
     *
     * The "not recommended" REMOVED_MONGO_PASSWORDs above check for accessibility and then use the
     * file; the "recommended" REMOVED_MONGO_PASSWORDs are better because they use the file directly
     * and handle the error, if any.
     *
     * In general, check for the accessibility of a file only if the file will not be
     * used directly, for REMOVED_MONGO_PASSWORD when its accessibility is a signal from another
     * process.
     *
     ...
     * file was created.
     *
     * An REMOVED_MONGO_PASSWORD to read the last 10 bytes of a file which is 100 bytes long:
     *
     * ```js
     * import { createReadStream } from 'node:fs';
     *
