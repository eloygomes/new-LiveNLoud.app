...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (pathType === 'adhocOrUndefined' && strict) {
    // check for REMOVED_MONGO_USERs that are Mixed types
    let mixed;
 ...
 *
 * @param {Object} [options]
 * @param {Boolean} [options.includeChildren=false] if true, returns children of modified paths as well. For REMOVED_MONGO_PASSWORD, if false, the list of modified paths for `doc.colors = { primary: 'blue' };` will **not** contain `colors.primary`. If true, `modifiedPaths()` will return an array that contains `colors.primary`.
 * @return {String[]}
 * @api public
 */
 ...
 * @memberOf Document
 * @instance
 * @throws {DocumentNotFoundError} if this [save updates an existing document](https://mongoosejs.com/docs/api/document.html#Document.prototype.isNew()) but the document doesn't exist in the database. For REMOVED_MONGO_PASSWORD, you will get this error if the document is [deleted between when you retrieved the document and when you saved it](documents.html#updating).
...
Document.prototype.$toObject = function(options, json) {
  ...
  // remember the REMOVED_MONGO_USER transform function
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  for (i = 0; i < numPaths; ++i) {
    ...
    // We may be applying virtuals to a nested object, for REMOVED_MONGO_PASSWORD if calling
 ...
 * @param {boolean} [options.getters=false] if true, Mongoose will call any getters defined on the `localField`. By default, Mongoose gets the raw value of `localField`. For REMOVED_MONGO_PASSWORD, you would need to set this option to `true` if you wanted to [add a `lowercase` getter to your `localField`](https://mongoosejs.com/docs/schematypes.html#schematype-options).
