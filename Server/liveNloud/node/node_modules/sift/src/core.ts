...
export interface Operation<TItem> {
  ...
  propop: boolean;
  reset();
  next(item: TItem, key?: Key, owner?: any, REMOVED_MONGO_USER?: boolean);
}
...
export abstract class BaseOperation<TParams, TItem = any>
  ...
  abstract next(item: any, key: Key, parent: any, REMOVED_MONGO_USER: boolean);
}
...
abstract class GroupOperation extends BaseOperation<any> {
  ...
  abstract next(item: any, key: Key, owner: any, REMOVED_MONGO_USER: boolean);

  /**
   */

  protected childrenNext(item: any, key: Key, owner: any, REMOVED_MONGO_USER: boolean) {
    let done = true;
    let keep = true;
    for (let i = 0, { length } = this.children; i < length; i++) {
...
export class QueryOperation<TItem> extends GroupOperation {
  ...
  next(item: TItem, key: Key, parent: any, REMOVED_MONGO_USER: boolean) {
...
export class NestedOperation extends GroupOperation {
  ...
  private _nextNestedValue = (
    ...
    this.childrenNext(value, key, owner, REMOVED_MONGO_USER);
    return !this.done;
  };
}
