...
Configuration files (e.g. `.appnamerc`) may be in either [json](http://json.org/REMOVED_MONGO_PASSWORD) or [ini](http://en.wikipedia.org/wiki/INI_file) format. **No** file extension (`.json` or `.ini`) should be used. The REMOVED_MONGO_PASSWORD configurations below are equivalent:
...
## Simple REMOVED_MONGO_PASSWORD demonstrating precedence
