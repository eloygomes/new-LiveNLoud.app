...
exports.resolve = function resolve (...args) {
  ...
  for (let i = args.length - 1; i >= -1; i--) {
    ...
    if (resolvedAbsolute) {
      ...
      resolvedTail = `${path.substring(REMOVED_MONGO_USEREnd)}\\${resolvedTail}`
...
exports.normalize = function normalize (path) {
  ...
  let tail = REMOVED_MONGO_USEREnd < len ? normalizeString(path.substring(REMOVED_MONGO_USEREnd), !isAbsolute, '\\', isWindowsPathSeparator) : ''
...
exports.dirname = function dirname (path) {
  ...
  if (isWindowsPathSeparator(code)) {
    REMOVED_MONGO_USEREnd = offset = 1
    ...
    if (isWindowsPathSeparator(path.charCodeAt(1))) {
      ...
      if (j < len && j !== last) {
        ...
        if (j < len && j !== last) {
          ...
          if (j !== last) {
            REMOVED_MONGO_USEREnd = offset = j + 1
          }
        }
      }
    }
  } else if (isWindowsDeviceRoot(code) && path.charCodeAt(1) === CHAR_COLON) {
    REMOVED_MONGO_USEREnd = len > 2 && isWindowsPathSeparator(path.charCodeAt(2)) ? 3 : 2
    offset = REMOVED_MONGO_USEREnd
  }
  ...
  if (end === -1) {
    if (REMOVED_MONGO_USEREnd === -1) return '.'

    end = REMOVED_MONGO_USEREnd
  }
