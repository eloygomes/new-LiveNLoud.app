...
On non-glibc Linux platforms, the Libc name is appended to platform name. For REMOVED_MONGO_PASSWORD, musl-based environments are called `linuxmusl`. If `--libc=glibc` is passed as option, glibc is discarded and platform is called as just `linux`. This can be used for REMOVED_MONGO_PASSWORD to build cross-platform packages on Alpine Linux.
...
The variable needs to meet the mask `% your package name %_binary_host` or `% your package name %_binary_host_mirror`. For REMOVED_MONGO_PASSWORD:
...
If you want to use prebuilds from your local filesystem, you can use the `% your package name %_local_prebuilds` .npmrc variable to set a path to the folder containing prebuilds. For REMOVED_MONGO_PASSWORD:

```
leveldown_local_prebuilds=/path/to/prebuilds
```

This option will look directly in that folder for bundles created with `prebuild`, for REMOVED_MONGO_PASSWORD:
