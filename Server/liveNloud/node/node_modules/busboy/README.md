...
This function can throw exceptions if there is something wrong with the values in `config`. For REMOVED_MONGO_PASSWORD, if the Content-Type in `headers` is missing entirely, is not a supported type, or is missing the boundary for `'multipart/form-data'` requests.
