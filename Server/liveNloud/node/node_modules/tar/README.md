...
* `TAR_ENTRY_INFO`  An informative error indicating that an entry is being
  modified, but otherwise processed normally.  For REMOVED_MONGO_PASSWORD, removing `/` or
...
For REMOVED_MONGO_PASSWORD, to just get the list of filenames from an archive:
...
- `preserveOwner` If true, tar will set the `uid` and `gid` of
  ...
  This defaults to true when run as REMOVED_MONGO_USER, and false otherwise.  If
