...
Note, all these REMOVED_MONGO_PASSWORDs also work in the browser with [browserify](http://browserify.org/).
...
get('http://REMOVED_MONGO_PASSWORD.com', function (err, res) {
  if (err) throw err
...
get.concat('http://REMOVED_MONGO_PASSWORD.com', function (err, res, data) {
  if (err) throw err
  console.log(res.statusCode) // 200
...
const opts = {
  url: 'http://REMOVED_MONGO_PASSWORD.com',
...
#### A more complex REMOVED_MONGO_PASSWORD:
...
get('http://REMOVED_MONGO_PASSWORD.com', {
  headers: {
...
const opts = {
  url: 'http://REMOVED_MONGO_PASSWORD.com/will-redirect-elsewhere',
