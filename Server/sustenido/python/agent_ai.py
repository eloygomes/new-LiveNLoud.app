...
_CHORD_RE = re.compile(
    r"(?<!\w)[A-G](?:[#b])?(?:m|min|maj|sus|dim|aug|add|7M|º)?\d*(?:/[A-G](?:[#b])?)?(?!\w)"
)

# _SECTION_RE = re.compile(
#     r"^\s*(?:\[\s*(?:intro|tab\s*-\s*(?:intro|passagem|estrofes|variação)|primeira\s+parte|segunda\s+parte|solo\s+intro)\s*\]|par?te\s+\d+\s+de\s+\d+|h\.?n\.?(?:\s*\.)*|\(\s*(?:passagem|\d+(?:/\d+)?)\s*\))\s*$",
#     re.IGNORECASE,
# )

_SECTION_RE = re.compile(
    r"^\s*(?:\[\s*(?:intro|tab\s*-\s*(?:intro|passagem|estrofes|variação)|\w+\s+parte|solo\s+intro)\s*\]"  
        r"|par?te\s+\d+\s+de\s+\d+"                                       
        r"|h\.?n\.?(?:\s*\.)*"                                            
        r"|\(\s*(?:passagem|\d+(?:/\d+)?)\s*\)"                           
        r")\s*$",
    re.IGNORECASE,
...
#     for raw in lines:
#         plain = re.sub(r"<[^>]+>", "", raw).rstrip()

#         if not plain or _META_RE.match(plain) or _SECTION_RE.match(plain):
#             out.append("")
#             continue

#         if _is_tab_line(plain):
#             out.append("")
#             continue

#         # only_chords = _CHORD_RE.search(plain) and _CHORD_RE.sub("", plain).strip() == ""
#         only_chords = (
#             (_CHORD_RE.search(plain) and _CHORD_RE.sub("", plain).strip() == "")     # linha só de acordes
#             or re.fullmatch(                                                         # (passagem), (4), (4/9), (( 4/9 ))
#                 r"\s*\(+\s*(?:passagem|\d+(?:/\d+)?)\s*\)+\s*", plain, flags=re.I
#             )
#             or re.fullmatch(r"\s*\(+\s*\)?\s*", plain)                               # casos como "(   ("
#         )
#         if only_chords:
#             out.append("")
#             continue

#         lyric = _CHORD_RE.sub("", plain).strip()
#         out.append(lyric)
...
def _strip_chords(cifra: str) -> str:
    ...
    for raw in lines:
        # strip HTML tags and trailing whitespace
        plain = re.sub(r"<[^>]+>", "", raw).rstrip()

        # skip lines that are exactly "(4/9)" or any "(number/number)"
        if _FRACTION_RE.match(plain):
            out.append("")
            continue

        # skip empty, metadata or section headers
        if not plain or _META_RE.match(plain) or _SECTION_RE.match(plain):
            out.append("")
            continue
        ...
        if _CHORD_RE.search(plain) and _CHORD_RE.sub("", plain).strip() == "":
            out.append("")
            continue


        # skip lines like "(   ("
        if re.fullmatch(r"\s*\(+\s*\(?\s*\)?\s*\)+\s*", plain):
            out.append("")
            continue

        # finally, remove any remaining chord tokens and keep the lyric
        lyric = _CHORD_RE.sub("", plain).strip()
        out.append(lyric)
...
#     # tenta o LLM; em caso de quota excedida usa fallback local
#     try:
#         raw  = EXECUTOR.invoke({"query": f"Separe letra, acordes e TABs de {key}"})
#         resp = parser.parse(raw["output"])
#     except openai.RateLimitError as err:
#         logging.warning("[agent] quota exceeded – fallback local: %s", err)
#         resp = SongCifraResponse(
#             artist_name=artist_name,
#             song_title=song_title,
#             song_cifra=song_cifra or "",
#         )

#     # Se o modelo não retornou a cifra mas recebemos via parâmetro, usa-a
#     if not resp.song_cifra and song_cifra:
#         resp.song_cifra = song_cifra

#     # Fallback absoluto: busca em userData se ainda estiver vazio
...
def agent(
    ...
    # Always use LLM to split; remove fallback to local parsing and helpers
    # Invoke the tool-calling agent to get separated JSON fields
    raw = EXECUTOR.invoke({"query": f"Separe letra, acordes e TABs de {key}"})
    resp = parser.parse(raw["output"])
