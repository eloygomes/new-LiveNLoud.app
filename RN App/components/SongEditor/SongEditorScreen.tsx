...
const SongEditorScreen = ({ mode }: Props) => {
  ...
  const applySongDoc = useCallback((doc?: SongDoc | null) => {
    ...
    if (Array.isArray(doc.embedVideos) && doc.embedVideos.length) {
      setVideos((current) => uniqueStrings([...current, ...doc.embedVideos]));
    }
    if (Array.isArray(doc.setlist) && doc.setlist.length) {
      setSetlists((current) => uniqueStrings([...current, ...doc.setlist]));
      setSetlistOptions((current) => uniqueStrings([...current, ...doc.setlist]));
    }
  }, []);
        ...
        >
          ...
          <View style={styles.insightsRow}>
            <View style={styles.insightCard}>
              ...
              <Text style={styles.insightValue}>{overallProgress}%</Text>
            </View>
            <View style={styles.insightCard}>
              <Text style={styles.insightLabel}>Instruments</Text>
              <Text style={styles.insightValue}>{activeInstrumentCount}/6</Text>
            </View>
            <View style={styles.insightCard}>
              <Text style={styles.insightLabel}>Videos</Text>
              <Text style={styles.insightValue}>{videos.length}</Text>
            </View>
          </View>
              ...
              {instrumentConfig.map((instrument) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={styles.instrumentStatus}>
                      {isBusy
                        ? "Scraping..."
                        : hasLink
                          ? "Link added"
                    ...
                    <Text style={styles.instrumentPreview} numberOfLines={1}>
                      {instrumentLinks[instrument.key] || "No URL yet"}
                    </Text>
                  </TouchableOpacity>
                );
              })}
          ...
          <View style={styles.panelCard}>
            ...
            <TouchableOpacity style={styles.utilityCard} onPress={openVideoSheet}>
              ...
              <View style={styles.utilityBody}>
                ...
                <Text style={styles.utilitySubtitle}>
                  Add one or more video URLs for this song
                </Text>
            ...
            <TouchableOpacity style={styles.utilityCard} onPress={openSetlistSheet}>
              ...
              <View style={styles.utilityBody}>
                ...
                <Text style={styles.utilitySubtitle}>
                  Open the sheet to select or create song tags
                </Text>
                <Text style={styles.utilityMeta}>
