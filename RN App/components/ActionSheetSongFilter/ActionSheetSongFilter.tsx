...
const ActionSheetSongFilter = forwardRef<ActionSheetRef, Props>(
  (
    ...
    useEffect(() => {
      loadSelectedSetlists().then((stored) => {
        if (stored.length && selectedSetlists.length === 0) {
          setSelectedSetlists(stored);
        }
      });
    }, [selectedSetlists.length, setSelectedSetlists]);
    ...
    const toggleTag = (tag: string) => {
      ...
      saveSelectedSetlists(updated).catch((err) =>
    ...
    const saveTagChanges = async () => {
      ...
      try {
        ...
        await saveSelectedSetlists(nextSelected);
        setPendingRemovals([]);
        setIsEditing(false);
      } catch {
          ...
          <View style={styles.sectionCard}>
            ...
            <View style={styles.buttonRow}>
              <TouchableOpacity style={[styles.actionButton, styles.disabledAction]}>
                <FontAwesome5 name="spotify" size={14} color="#8f8f8f" brands />
                <Text style={styles.disabledActionText}>Spotify</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.actionButton, styles.disabledAction]}>
                <FontAwesome5 name="youtube" size={14} color="#8f8f8f" brands />
