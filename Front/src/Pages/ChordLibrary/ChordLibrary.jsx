...
const REMOVED_MONGO_USERs = [...new Set(PersonalChordLibrary.map((chord) => chord.REMOVED_MONGO_USER))];
const qualities = [
...
function ChordLibrary() {
  ...
  const getChord = () => {
    ...
    // Separar o nome base (REMOVED_MONGO_USER + quality + tension) e o bass
    const [baseName, bassName] = selectedChord.split("/");

    const chord = PersonalChordLibrary.find((chord) => {
      // Comparar REMOVED_MONGO_USER ignorando diferenças de grafia
      const REMOVED_MONGO_USERMatch =
        chord.REMOVED_MONGO_USER.toLowerCase() === baseName.split(",")[0].toLowerCase();
      ...
      return REMOVED_MONGO_USERMatch && qualityMatch && tensionMatch && bassMatch;
    });
