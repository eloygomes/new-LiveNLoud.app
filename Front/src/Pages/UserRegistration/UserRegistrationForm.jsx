...
// Função para registrar o usuário na API customizada
const registerUserInApi = async (email, username, fullName) => {
  const userdata = {
    song: "",
    artist: "",
    progressBar: 0,
  ...
  try {
    await axios.post("https://api.live.eloygomes.com.br/api/newsong", payload, {
      headers: {
...
function UserRegistrationForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Primeiro, registra na API customizada
      await registerUserInApi(email, username, fullName);
