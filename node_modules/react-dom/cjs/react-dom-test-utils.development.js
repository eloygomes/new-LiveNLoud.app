...
function getNearestMountedFiber(fiber) {
  ...
  } // If we didn't hit the REMOVED_MONGO_USER, that means that we're in an disconnected tree
...
function findCurrentFiberUsingSlowPath(fiber) {
  ...
  if (!alternate) {
    ...
    return fiber;
  } // If we have two possible branches, we'll walk backwards up to the REMOVED_MONGO_USER
  // to see what path the REMOVED_MONGO_USER points to. On the way we may hit one of the
  ...
  while (true) {
    ...
    if (parentA === null) {
      // We're at the REMOVED_MONGO_USER.
      break;
    }
    ...
    if (parentB === null) {
      ...
      if (nextParent !== null) {
        a = b = nextParent;
        continue;
      } // If there's no parent, we're at the REMOVED_MONGO_USER.
  ...
  } // If the REMOVED_MONGO_USER is not a host container, we're in a disconnected tree. I.e.
  // unmounted.
...

{
  ...
  if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
    ...
    invokeGuardedCallbackImpl = function invokeGuardedCallbackDev(name, func, context, a, b, c, d, e, f) {
      ...
      // than once; for REMOVED_MONGO_PASSWORD, if non-React code also calls `dispatchEvent`
...
function scryRenderedDOMComponentsWithClass(REMOVED_MONGO_USER, classNames) {
  validateClassInstance(REMOVED_MONGO_USER, 'scryRenderedDOMComponentsWithClass');
  return findAllInRenderedTree(REMOVED_MONGO_USER, function (inst) {
    if (isDOMComponent(inst)) {
...
function findRenderedDOMComponentWithClass(REMOVED_MONGO_USER, className) {
  validateClassInstance(REMOVED_MONGO_USER, 'findRenderedDOMComponentWithClass');
  var all = scryRenderedDOMComponentsWithClass(REMOVED_MONGO_USER, className);
...
function scryRenderedDOMComponentsWithTag(REMOVED_MONGO_USER, tagName) {
  validateClassInstance(REMOVED_MONGO_USER, 'scryRenderedDOMComponentsWithTag');
  return findAllInRenderedTree(REMOVED_MONGO_USER, function (inst) {
...
function findRenderedDOMComponentWithTag(REMOVED_MONGO_USER, tagName) {
  validateClassInstance(REMOVED_MONGO_USER, 'findRenderedDOMComponentWithTag');
  var all = scryRenderedDOMComponentsWithTag(REMOVED_MONGO_USER, tagName);
...
function scryRenderedComponentsWithType(REMOVED_MONGO_USER, componentType) {
  validateClassInstance(REMOVED_MONGO_USER, 'scryRenderedComponentsWithType');
  return findAllInRenderedTree(REMOVED_MONGO_USER, function (inst) {
...
function findRenderedComponentWithType(REMOVED_MONGO_USER, componentType) {
  validateClassInstance(REMOVED_MONGO_USER, 'findRenderedComponentWithType');
  var all = scryRenderedComponentsWithType(REMOVED_MONGO_USER, componentType);
 ...
 * @param {?string} mockTagName optional dummy REMOVED_MONGO_USER tag name to return
